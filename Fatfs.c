...
/-----------------------------------------------------------------------------/
 FatFs是一个通用的文件系统模块，用于在小型嵌入式系统中实现FAT文件系统,提供下面的函数：
...
因为FatFs模块完全与磁盘I/O层分开，因此需要下面的函数来实现底层物理磁盘的读写与获取当前时间。
...
#include "common.h"		/* Declarations of low level disk I/O functions */
#include "MMC.h"		/* Declarations of low level disk I/O functions */
#include "RTC.h"		/* Declarations of low level disk I/O functions */
#include "Uart.h"		/* Declarations of low level disk I/O functions */
...
UINT bw, br, i;
char filename[20];
__no_init char Msg2Store[256];//信息帧要存入SD卡

//fatfs=====================================================================

//往sd卡存储数据需要耗时100ms左右
void MassStorage(void){  		
    int  n_f_puts =0;//检查写一次文件内容字节,如果比写入的内容少,说明文件满了
    rc = f_open(&fil, "MESSAGE.TXT", FA_WRITE| FA_OPEN_ALWAYS);if(rc==FR_OK){}  
    rc = f_lseek(&fil,fil.fsize);           //Move to end of the file to append data    
    sprintf(Msg2Store,"20%02u/%02u/%02u\t%02u:%02u:%02u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%u\t%u\t%d\r\n",     
              //年月日 时分秒  //电压 风速 风向  气温 气压 湿度   //水温 罗盘    //海流cell 1 2 3 4 5  //波高 波周期 波向
            FM3130.year,FM3130.month,FM3130.day,FM3130.hour,FM3130.min,FM3130.sec, 
            VCC.value,WindSpeed.value,WindDirection.value,AirTemperature.value,Barometer.value,Humidity.value,            
            WaterTemperature.value,Heading.value,
            VE1.value,VN1.value,VU1.value,VE2.value,VN2.value,VU2.value,VE3.value,VN3.value,VU3.value,VE4.value,VN4.value,VU4.value,VE5.value,VN5.value,VU5.value,
            WaveHeight.value,WavePeriod.value,WaveDirection.value);           
    n_f_puts = f_puts (Msg2Store, &fil);//从文件内数据的最后写入字符串      
    if(n_f_puts <1){PrtStr2Teminal( "SDcard error\r\n",2);}             //判断磁盘写 是否成功                 
    rc = f_sync(&fil);/*f_sync函数和f_close函数执行同样的过程，但是文件仍处于打开状态，并且可以继续对文件执行读/写/移动指针操作。
...
void Setup_MassStorage(void){
    ...
    if(rc==FR_OK){
        rc = f_lseek(&fil,fil.fsize);   //移动文件指针/* Move to end of the file to append data */
        sprintf(Msg2Store, "DAQ OS start at 20%02d/%02d/%02d\t%02d:%02d\r\n\r\n年/月/日\t时:分:秒\t电压\t风速\t风向\t气温\t气压\t湿度\t水温\t罗盘\tVE1\tVN1\tVU1\tVE2\tVN2\tVU2\tVE3\tVN3\tVU3\tVE4\tVN4\tVU4\tVE5\tVN5\tVU5\t波高\t波周期\t波向\r\n", FM3130.year,FM3130.month,FM3130.day,FM3130.hour,FM3130.min); //0x0d 0x0a
        f_puts (Msg2Store, &fil);	//从文件内数据的最后写入字符串     
        //f_printf(&fil,"%D\r\n",1000);	//从文件内数据的最后写入字符串   
        rc = f_close(&fil);//Close the file.
    }else{
        //sprintf(BuoyCenterData, "***Warining! SD card open error! Error code: %d***\r\n", rc); //0x0d 0x0a
        //PrtStr2Teminal(BuoyCenterData,0);
    }
}
/*-----------------------------------------------------------------------*/
/* String functions                                                      */
